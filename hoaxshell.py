...
parser.add_argument("-w","--write",action="store",help="writes the payload to a .ino file")
	...
	try:
		...
		if not args.grab:
			...
			if not args.raw_payload:
				encoded_payload=encodePayload(payload)
			...
			if write:
				f=open(args.write+".ino", mode='w')
				tmp=open("./arduino_template/default.txt", mode='r')
				content=tmp.read().replace("PAYLOAD_HERE",encoded_payload)
				f.write(content)
				tmp.close()
				f.close()
